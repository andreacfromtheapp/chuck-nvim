...
local function set_action(line)
  local action
  if string.match(line, "sporking incoming shred: %d") then action = "add" end
  if string.match(line, "replacing shred %d") then action = "replace" end
  if string.match(line, "removing shred: %d") then action = "remove" end
...
function M.set_table(line)
  ...
  if action ~= nil then
    ...
    for n = 1, count + 1 do
      ...
      if id ~= nil then shred_id = tostring(id) end
      -- if name is valid initialize shred_name
      if name ~= nil then shred_name = name end
      ...
      if shred_id ~= nil and shred_name ~= nil and shred_name:match ".ck" then
        ...
        if action == "add" or "replace" then M.shreds_table[shred_id] = shred_name end
        -- if removing set shred as nil: id is key -> nil
        if action == "remove" then M.shreds_table[shred_id] = nil end
      end
    end
  end
end
